...
/**
 * @author Wenjie Peng
 * @create 2024-12-19 20:13
 * @description
 */
public abstract class ThreeIntValueBox extends FormatAttrBox<IntList, ThreeIntValueBox> {
    private Int32ListBox container = new Int32ListBox();
    private BytesSplitter splitter = new BytesSplitter(Constant.COMMA);

    public ThreeIntValueBox() {
        value = new IntList();
    }
    ...
    @Override
    public abstract ThreeIntValueBox newInstance();

    @Override
    public ThreeIntValueBox char2Object(String s) {
        if (value == null) {
            value = new IntList();
        } else {
            value.clear();
    ...
    @Override
    public ThreeIntValueBox decode(Bytes iByteCode) {
    ...
    public int get(int sampleIndex, int valueIndex) {
        return value.fastGet(sampleIndex * 3 + valueIndex);
    }
    ...
    @Override
    public ThreeIntValueBox char2Object(Bytes bytes, boolean b) {
        return decode(bytes);
    }
    ...
    public Object getObjectByIndividualIndex(int index) {
        return IntList.wrap(
                value.fastGet(3 * index),
                value.fastGet(3 * index + 1),
                value.fastGet(3 * index + 2)
        );
    }
}
